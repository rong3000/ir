...
import 'package:intelligent_receipt/pages/receipts_page/my_paginated_data_table.dart';
...
class DessertDataSource extends DataTableSource {
  ...
  onSelectedRow(bool selected, Dessert dessert) {
    if (selected) {
      _selectedDesserts.add(dessert);
      notifyListeners();
    } else {
      _selectedDesserts.remove(dessert);
      notifyListeners();
    }
    ;
  }
  ...
  deleteSelected() {
    if (_selectedDesserts.isNotEmpty) {
      List<Dessert> temp = [];
      temp.addAll(_selectedDesserts);
      for (Dessert dessert in temp) {
  ...
  DataRow getRow(int index) {
    ...
    return DataRow.byIndex(
      index: index,
      selected: _selectedDesserts.contains(dessert),
      onSelectChanged: (b) {
        print("${dessert.name} is Onselect");
        onSelectedRow(b, dessert);
      },
      cells: <DataCell>[
        DataCell(
          Text('${dessert.name}'),
        ...
        DataCell(
          Text('${dessert.fat.toStringAsFixed(1)}'),
          showEditIcon: true,
        ),
        DataCell(
          Text('${dessert.carbs}'),
        ...
        DataCell(
          Text('${dessert.protein.toStringAsFixed(1)}'),
          showEditIcon: true,
        ),
        DataCell(
          Text('${dessert.sodium}'),
...
class _DataTableDemoState extends State<DataTableDemo> {
  ...
  @override
  void initState() {
    _dessertsDataSource._selectedDesserts = [];
    super.initState();
  }
        ...
        child: ListView(
          children: <Widget>[
            ...
            Row(
              ...
              children: <Widget>[
                OutlineButton(
                  child: Text('SELECTED ${_dessertsDataSource._selectedDesserts.length}'),
                  onPressed: () {},
                ),
                OutlineButton(
